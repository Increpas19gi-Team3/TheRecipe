...
<title> 스프링 게시판 </title>
	<script type="text/javascript">
		...
		function change_GUBUN() {
			...
			location.href='NoticeList.do?sortColumn='+sortColumn+'&orderby='+orderby+'&whereColumn='+whereColumn+'&word='+word+'&GUBUN='+GUBUN+'&pageCutCount='+pageCutCount;
		}
						...
						<c:when test="${sortColumn eq 'e_nttilte'}"><!-- 제목 정렬일때 -->
							<c:choose>
								<c:when test="${orderby eq 'ASC' }">
									<a href="NoticeList.do?sortColumn=e_nttilte&orderby=DESC&whereColumn=${whereColumn }&word=${word }&GUBUN=${GUBUN }&pageCutCount=${pageCutCount }">제목 ▲</a>
								</c:when>
								<c:otherwise>
									<a href="NoticeList.do?sortColumn=e_nttilte&orderby=ASC&whereColumn=${whereColumn }&word=${word }&GUBUN=${GUBUN }&pageCutCount=${pageCutCount }">제목 ▼</a>									
								</c:otherwise>
							</c:choose>
						</c:when>
